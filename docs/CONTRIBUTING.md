...
This page is for people: the environment, the build, the test runs, the flow of a change and what a pull request has to carry. The repository map, the build-tag rules and the code review rules that coding agents read are in [AGENTS.md](https://github.com/coddy-project/coddy-agent/blob/main/AGENTS.md), and the detailed project rules live in `.cursor/rules/*.mdc` (mirrored to `.claude/rules/*.md`); this page links to them instead of repeating them.
...
The SPA does not need a Go rebuild per edit. Run an `http`-only backend on one port and Vite's dev server on another, as the **Dev workflow** section of [DESIGN.md](https://github.com/coddy-project/coddy-agent/blob/main/DESIGN.md) describes:
...
- **The harnesses in `examples/`** - real binaries against a reachable model: `./examples/build_coddy.sh`, then `./examples/test_acp.sh` (ACP over stdio), `./examples/test_httpserver.sh` (a disposable `coddy serve`), `./examples/test_cli.sh` (the console in a pty, Linux only) and `./examples/test_swarm.sh` (three relays, no model needed). The layout and every script are in [examples/README.md](https://github.com/coddy-project/coddy-agent/blob/main/examples/README.md).

The happy path of a feature is an executable Gherkin spec in the repo-root `features/` directory, run by a godog harness in the package that owns the behaviour (for example `external/httpserver/bdd_*_test.go`, with `Options.Paths` pointing at `../../features/<name>.feature`). Edge and error cases are ordinary unit tests next to the code, never scenarios. Specs stay deterministic and LLM-free through a stub runner, and they run under the tag that owns them as part of `make test`. Conventions are in [.claude/rules/testing.md](https://github.com/coddy-project/coddy-agent/blob/main/.claude/rules/testing.md).
...
The flow is red, green, then the checks - in full in [.claude/rules/workflow.md](https://github.com/coddy-project/coddy-agent/blob/main/.claude/rules/workflow.md):
...
- The SPA is formatted with Prettier (`npm run fmt` in `external/ui`); [DESIGN.md](https://github.com/coddy-project/coddy-agent/blob/main/DESIGN.md) is its contract for tokens, layout and component behaviour, and the localization rules (every dictionary changed in the same commit) are in [AGENTS.md](https://github.com/coddy-project/coddy-agent/blob/main/AGENTS.md).

The one-page version is [.claude/rules/code-style.md](https://github.com/coddy-project/coddy-agent/blob/main/.claude/rules/code-style.md).
...
| The repository map, build tags, the pre-commit gate, the code review rules | [AGENTS.md](https://github.com/coddy-project/coddy-agent/blob/main/AGENTS.md) |
| Tokens, layout and component contracts of the web UI | [DESIGN.md](https://github.com/coddy-project/coddy-agent/blob/main/DESIGN.md) |
...
| The full workflow, testing and style rules | [.claude/rules/workflow.md](https://github.com/coddy-project/coddy-agent/blob/main/.claude/rules/workflow.md), [.claude/rules/testing.md](https://github.com/coddy-project/coddy-agent/blob/main/.claude/rules/testing.md), [.claude/rules/code-style.md](https://github.com/coddy-project/coddy-agent/blob/main/.claude/rules/code-style.md) |
| Executable specs and end-to-end harnesses | `features/`, [examples/README.md](https://github.com/coddy-project/coddy-agent/blob/main/examples/README.md) |
| The documentation map | [docs/nav.yaml](https://github.com/coddy-project/coddy-agent/blob/main/docs/nav.yaml) |
