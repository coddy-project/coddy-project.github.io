...
- [`Dockerfile`](https://github.com/coddy-project/coddy-agent/blob/main/Dockerfile) - multi-stage build (**Node** UI bundle, **Go** binary, **`scratch`** runtime)
- [`docker-compose.yml`](https://github.com/coddy-project/coddy-agent/blob/main/docker-compose.yml) - run **`ghcr.io/coddy-project/coddy-agent`** (default **`docker compose`**)
- [`docker-compose.dev.yml`](https://github.com/coddy-project/coddy-agent/blob/main/docker-compose.dev.yml) - build from source, publish port **12345**, volumes
- [`.dockerignore`](https://github.com/coddy-project/coddy-agent/blob/main/.dockerignore) - keeps context small; never commit **`config.yaml`** with secrets
- [`examples/httpserver/docker.sh`](https://github.com/coddy-project/coddy-agent/blob/main/examples/httpserver/docker.sh) - automated smoke test

Published images: **[coddy-agent on GHCR](https://github.com/coddy-project/coddy-agent/pkgs/container/coddy-agent)** (`ghcr.io/coddy-project/coddy-agent`). CI builds **multi-arch** manifests (**`linux/amd64`**, **`linux/arm64`**) on SemVer tags and pushes floating aliases (**`latest`**, **`MAJOR.MINOR`**, **`MAJOR`**) when appropriate - see [`.github/workflows/docker-build-push.yaml`](https://github.com/coddy-project/coddy-agent/blob/main/.github/workflows/docker-build-push.yaml).
...
| File | When to use |
|------|-------------|
| [`docker-compose.yml`](https://github.com/coddy-project/coddy-agent/blob/main/docker-compose.yml) | Pull a release from GHCR (**`docker compose pull`**) - day-to-day and production-like runs without building on the host |
| [`docker-compose.dev.yml`](https://github.com/coddy-project/coddy-agent/blob/main/docker-compose.dev.yml) | **`build:`** from the repo [`Dockerfile`](https://github.com/coddy-project/coddy-agent/blob/main/Dockerfile) - hacking on Coddy, reproducing CI, or running **`examples/httpserver/docker.sh`** |
...
**`CODDY_BUILD_TAGS`** must stay comma-separated with **no spaces**, matching **`go build -tags=`**. The dev file defaults to **`http,scheduler,ui,memory,gateway`** (matching the [`Dockerfile`](https://github.com/coddy-project/coddy-agent/blob/main/Dockerfile) **`BUILD_TAGS`** default) so the built image can run the messenger gateway; drop **`gateway`** to trim it.
...
- The **published GHCR image carries the `gateway` tag** (CI [`docker-build-push.yaml`](https://github.com/coddy-project/coddy-agent/blob/main/.github/workflows/docker-build-push.yaml) sets **`BUILD_TAGS=http,scheduler,ui,memory,cli,gateway,swarm`**), so **`docker-compose.yml`** with **`CODDY_COMMAND=gateway`** runs on the stock image; no custom **`CODDY_IMAGE`** is needed.
...
- **`memory`** - long-term memory copilot and session memory REST (**[external/memory/README.md](https://github.com/coddy-project/coddy-agent/blob/main/external/memory/README.md)**); toggle runtime behavior via **`memory.enable`**.
