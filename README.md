...
| `compare/index.html` | Comparison page: Coddy against nineteen other agent harnesses, five tables |
| `ru/index.html`, `ru/compare/index.html` | Russian versions of the landing and comparison pages; generated and committed |
| `scripts/build_ru.py`, `scripts/ru_translations.tsv` | Build the Russian pages from the English HTML structure and human-edited translations |
...
English pages live at `/` and `/compare/`; their Russian counterparts live at `/ru/` and
`/ru/compare/`. The RU/EN links in the shared header keep visitors on the equivalent page and,
when JavaScript is available, preserve the current section anchor. Each page has its own canonical,
language alternates, and sitemap entry.

After changing either English page, update the Russian text in `scripts/ru_translations.tsv` and run:

```bash
python scripts/build_ru.py
```
